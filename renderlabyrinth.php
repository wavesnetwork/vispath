...
// loadSiblings, createTree: load the all nodes in the pathway from the db to create the structure of nodes 
    private function loadSiblings($selected) {
    	$query = "SELECT mn.id, mn.title, mn.text FROM map_node_links as mnl join map_nodes as mn on mn.id=mnl.node_id_2 WHERE mnl.node_id_1=".$selected['original_parent_id'];
   	
    	$this->db->setQuery($query);
    	$result = $this->db->loadObjectList();
    	$children = [];
    	foreach($result as $row) {
    		if ($row->id == $selected['original_id']) {
    			$children[] = $selected;
    		} else {
    			$children[] =  [
    				'id' => $row->id,
                    'name' => $this->stripName(strip_tags($row->title)),
                    'selected' => false,
                    'description' => strip_tags($row->text),
                    'parent' => $selected['parent'],
                    'parent_id' => $selected['parent_id'],
                    'original_parent_id' => $selected['original_parent_id'],
    			];
    		}
    	}
    	return $children;
    ...
    private function createTree(&$list, $parent) {
        ...
        foreach ($parent as $k=>$l){
            if(isset($list[$l['id']])){
                ...
                $l['children'] = $this->loadSiblings($l['children'][0]);
            }
            $tree[] = $l;
        } 
        return $tree;
    ...
    private function isLastNode($nodeId, $mapId) {
		$query = "select node_id_2 from `map_node_links` where map_id=".$mapId." and node_id_2 not in (select node_id_1 from map_node_links where map_id = ".$mapId.")";
		$this->db->setQuery($query);
		$result = $this->db->loadObjectList();
		foreach ($result as $row) {
			if ($row->node_id_2 == $nodeId) {
				return true;
			}
		}
		return false;
...
//identifies the last node to execute the function
            $isLast = $this->isLastNode($nodeId, $mapId);
            if ($isLast) {
                $sessionId = Session::instance()->get('session_id', NULL);
                $traces = DB_ORM::model('user_sessionTrace')->getTraceBySessionID($sessionId);
                $tree = [];
                $keys = [];
                $previous = null;
                $previous_key = $previous_original_key = 0;
                $multiplier = 1000000;
                if (!empty($traces)) {
                    foreach ($traces as $trace) {
                        $found = 1;
                        foreach ($keys as $key) {
                            if ($key == $trace->node->id) {
                                $found++;
                            }
                        }
                        $id = $found*$multiplier*$trace->node->id;
                        $keys[] = $trace->node->id;
                        $tree_node = [
                            'id' => $id,
                            'original_id' => $trace->node->id,
                            'name' => $this->stripName(strip_tags($trace->node->title)),
                            'selected' => true,
                            'description' => strip_tags($trace->node->text),
                            'parent' => '',
                            'parent_id' => 0,
                            'children' => [],
                        ]; 
                        if ($previous != null && $previous->node->id != $trace->node->id) {
                            $tree_node['parent'] = $this->stripName(strip_tags($previous->node->title));
                            $tree_node['parent_id'] = strip_tags($previous_key);
                            $tree_node['original_parent_id'] = $previous_original_key;
                        }
                        $previous = $trace;
                        $previous_key = $id;
                        $previous_original_key = $trace->node->id;
                        $tree[] = $tree_node;
                    }
                    $keys = [];
